 ...
 */
@RegisteredType(name="ExampleApplication")
public class ExampleApplication extends Application {
    /**
     * This is the constructor of the ExampleApplication class. When the
    ...
    public void init() {
        ...
        about.onActivate(_ -> onAboutAction());
        addAction(about);

        var preferences = new SimpleAction("preferences", null);
        preferences.onActivate(_ -> onPreferencesAction());
        addAction(preferences);

        var greet = new SimpleAction("greet", null);
        greet.onActivate(_ -> onGreetAction());
        addAction(greet);
    ...
    private void onAboutAction() {
        ...
        var about = AboutDialog.builder()
            ...
            .setApplicationIcon("org.domain.Example")
    ...
    private void onGreetAction() {
        var win = (ExampleWindow) this.getActiveWindow();
        if (win != null) {
            win.label.setLabel(i18n("Hello again!"));
        }
    }
}
